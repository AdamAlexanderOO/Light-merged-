...
export const Earth3D: React.FC<Earth3DProps> = ({
  ...
  const solarWindRef = useRef<THREE.Points | null>(null);
  const solarWindDataRef = useRef<{ pos: Float32Array; vel: Float32Array; life: Float32Array }>({
  ...
  useEffect(() => {
    ...
    const earthMat = new THREE.ShaderMaterial({
      uniforms: {
        ...
        u_vision_mode: { value: 0 },
        u_light_fusion_boost: { value: 1.5 },
        u_tactical_grid: { value: 0.7 }
      },
      vertexShader: `
      ...
      fragmentShader: `
        ...
        uniform int u_vision_mode;
        uniform float u_light_fusion_boost;
        uniform float u_tactical_grid;
        ...
        void main() {
          ...
          // MULTI-SPECTRAL VISION MODES (COMBAT DECK ENGINE)
          if (u_vision_mode == 1) {
            // LIGHT FUSION MODE: High dynamic range bloom, amplified solar reflections, glowing golden twilight rim
            vec3 lumaVal = vec3(dot(finalColor, vec3(0.299, 0.587, 0.114)));
            finalColor = mix(lumaVal, finalColor, 1.35);
            finalColor = pow(finalColor, vec3(0.92)) * (1.1 + u_light_fusion_boost * 0.25);
            if (umbraRingGlow > 0.0) {
              finalColor += vec3(1.0, 0.75, 0.25) * umbraRingGlow * 1.6 * u_light_fusion_boost;
            }
          } else if (u_vision_mode == 2) {
            // INFRARED / THERMAL FLIR: False-color surface temperature with umbral shadow cooling signature
            float temp = dot(finalColor, vec3(0.333)) * effectiveSunlight;
            temp = clamp(temp - shadowDarkness * 0.25, 0.0, 1.0);
            vec3 flirColor;
            if (temp < 0.25) {
              flirColor = mix(vec3(0.05, 0.05, 0.25), vec3(0.1, 0.3, 0.7), temp / 0.25);
            } else if (temp < 0.5) {
              flirColor = mix(vec3(0.1, 0.3, 0.7), vec3(0.1, 0.8, 0.6), (temp - 0.25) / 0.25);
            } else if (temp < 0.75) {
              flirColor = mix(vec3(0.1, 0.8, 0.6), vec3(0.9, 0.8, 0.1), (temp - 0.5) / 0.25);
            } else {
              flirColor = mix(vec3(0.9, 0.8, 0.1), vec3(1.0, 0.2, 0.1), (temp - 0.75) / 0.25);
            }
            finalColor = flirColor;
          } else if (u_vision_mode == 3) {
            // SDO HELIO / ULTRAVIOLET 171Å: Deep ultraviolet gold/violet solar dynamics false color
            float lumaUV = dot(finalColor, vec3(0.299, 0.587, 0.114));
            vec3 uvGold = vec3(0.95, 0.65, 0.15) * pow(lumaUV, 0.8) * 1.5;
            vec3 uvViolet = vec3(0.45, 0.1, 0.7) * (1.0 - lumaUV);
            finalColor = mix(uvViolet, uvGold, lumaUV) * (1.0 - shadowDarkness * 0.9);
          } else if (u_vision_mode == 4) {
            // TACTICAL COMBAT DECK: Vector wireframe grid, lat/lon geodesics, and holographic HUD isolines
            float latGrid = abs(sin(vUv.y * 36.0 * 3.14159));
            float lonGrid = abs(sin(vUv.x * 72.0 * 3.14159));
            float gridLines = smoothstep(0.96, 0.99, latGrid) + smoothstep(0.96, 0.99, lonGrid);
            
            vec3 tacticalBase = mix(vec3(0.02, 0.05, 0.08), vec3(0.05, 0.18, 0.28), effectiveSunlight);
            vec3 gridColor = vec3(0.0, 0.95, 1.0);
            finalColor = mix(tacticalBase, gridColor, gridLines * u_tactical_grid * 0.85);
            if (shadowDarkness > 0.5) {
              finalColor = mix(finalColor, vec3(0.95, 0.2, 0.2), (shadowDarkness - 0.5) * 0.8);
            }
          }
    ...
    // Coronal bloom sprite for Sun
    const sunCanvas = document.createElement('canvas');
    sunCanvas.width = 256;
    sunCanvas.height = 256;
    const sunCtx = sunCanvas.getContext('2d')!;
    const sunHalo = sunCtx.createRadialGradient(128, 128, 10, 128, 128, 128);
    sunHalo.addColorStop(0, 'rgba(255, 255, 255, 1)');
    sunHalo.addColorStop(0.2, 'rgba(253, 230, 138, 0.9)');
    sunHalo.addColorStop(0.5, 'rgba(245, 158, 11, 0.35)');
    sunHalo.addColorStop(1, 'rgba(245, 158, 11, 0)');
    sunCtx.fillStyle = sunHalo;
    ...
    // 7. Interplanetary Solar Wind Plasma Stream (Canvas & WebGL Engine)
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      pPositions[i * 3 + 0] = (Math.random() - 0.5) * 700;
      pPositions[i * 3 + 1] = (Math.random() - 0.5) * 700;
      pPositions[i * 3 + 2] = (Math.random() - 0.5) * 700;
      pLifetimes[i] = Math.random();
    }

    const windGeo = new THREE.BufferGeometry();
    windGeo.setAttribute('position', new THREE.BufferAttribute(pPositions, 3));
    ...
    const solarWindPoints = new THREE.Points(windGeo, windMat);
    scene.add(solarWindPoints);
    solarWindRef.current = solarWindPoints;
    solarWindDataRef.current = { pos: pPositions, vel: pVelocities, life: pLifetimes };
    ...
    const animate = () => {
      ...
      // Animate solar wind particles streaming toward Earth
      if (solarWindRef.current && sunLightRef.current) {
        const pArray = solarWindDataRef.current.pos;
        const sunPos = sunLightRef.current.position.clone().normalize();
        const count = pArray.length / 3;
        ...
        for (let i = 0; i < count; i++) {
          ...
          // Bow shock deflection around Earth's magnetosphere
          if (r < EARTH_RADIUS * 1.35) {
            px += (px / (r + 0.1)) * 3.8;
            py += (py / (r + 0.1)) * 3.8;
            pz += (pz / (r + 0.1)) * 3.8;
          }

          // Reset if passed behind Earth or drifted out of bounds
          if (r < EARTH_RADIUS * 0.95 || r > 850) {
            const offsetDist = 580 + Math.random() * 180;
            const spreadX = (Math.random() - 0.5) * 180;
            const spreadY = (Math.random() - 0.5) * 180;
            px = sunPos.x * offsetDist + spreadX;
            py = sunPos.y * offsetDist + spreadY;
            pz = sunPos.z * offsetDist + spreadX;
          }
  ...
  useEffect(() => {
    ...
    if (earthMeshRef.current && earthMeshRef.current.material instanceof THREE.ShaderMaterial) {
      ...
      // Update vision mode & optics config uniforms
      const modeInt = visionMode === 'light-fusion' ? 1 : visionMode === 'infrared' ? 2 : visionMode === 'sdo-uv' ? 3 : visionMode === 'tactical' ? 4 : 0;
      earthMeshRef.current.material.uniforms.u_vision_mode.value = modeInt;
      if (opticsConfig) {
        earthMeshRef.current.material.uniforms.u_light_fusion_boost.value = opticsConfig.lightFusionBoost;
        earthMeshRef.current.material.uniforms.u_tactical_grid.value = opticsConfig.tacticalGrid;
        earthMeshRef.current.material.uniforms.u_ocean_specular.value = 0.15 * opticsConfig.oceanSpecular;
        earthMeshRef.current.material.uniforms.u_night_brightness.value = opticsConfig.nightLights;
        earthMeshRef.current.material.uniforms.u_terminator_glow.value = 0.12 * opticsConfig.sunsetGlow;
        earthMeshRef.current.material.uniforms.u_umbra_ring_glow.value = 0.15 * opticsConfig.lightFusionBoost;
      }
    ...
    // Update Sun Light and 3D Sun direction
    const subSolar = getSubSolarPoint(currentTimestamp);
    ...
    if (sunGroupRef.current) {
      const sunFarVec = latLonToVector3(subSolar.lat, subSolar.lon, 720);
      sunGroupRef.current.position.set(...sunFarVec);
    }
  ...
  }, [currentTimestamp, selectedStation, cameraMode, cameraResetTrigger, showPathLine, showPenumbra, showDayNightTerminator, showCelestialIcons, visionMode, opticsConfig]);
