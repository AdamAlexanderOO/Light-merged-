import { LatLon, TelemetryReadout, CountryTimeInfo, ObservationStation, CombatDeckTelemetry } from '../types';
...
/**
 * Calculate Combat Deck live tactical telemetry (mach speed, space weather, intercept phase)
 */
export function calculateCombatDeckTelemetry(currentTimestamp: number): CombatDeckTelemetry {
  const p1 = getUmbraPosition(currentTimestamp);
  const p2 = getUmbraPosition(currentTimestamp + 15);
  let umbraSpeedKmh = 2850;
  if (p1 && p2) {
    const distKm = calculateDistanceKm(p1, p2);
    umbraSpeedKmh = Math.round((distKm / 15) * 3600);
  } else if (!p1) {
    umbraSpeedKmh = 0;
  }
  const speedOfSoundKmh = 1225; // Sea-level / atmospheric standard
  const machNumber = Math.round((umbraSpeedKmh / speedOfSoundKmh) * 100) / 100;

  // Space weather parameters (Solar Cycle 25 maximum conditions August 2026)
  const solarFluxSfu = 148 + Math.round(Math.sin(currentTimestamp * 0.001) * 6);
  const solarWindSpeedKmS = 438 + Math.round(Math.cos(currentTimestamp * 0.0008) * 16);
  const geomagneticKp = 2.4;

  let interceptStatus: CombatDeckTelemetry['interceptStatus'] = 'STANDBY';
  if (currentTimestamp >= 61200 && currentTimestamp <= 63000) {
    interceptStatus = 'APPROACH';
  } else if (currentTimestamp > 63000 && currentTimestamp <= 66750) {
    interceptStatus = 'LOCK';
  } else if (currentTimestamp > 66750) {
    interceptStatus = 'EGRESS';
  }
